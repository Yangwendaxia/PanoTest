	...
	public void onCreate(Bundle savedInstanceState)
	{
        ...
        this.setListener(new PLViewListener()
        {
			...
			public void onTouchesEnded(PLIView view, List<UITouch> touches, MotionEvent event)
			{
				...
				Log.d("MainActivity", "(x,y,z)"+ x+ "," + y +","+ z + " py = (" + py[0] +","+py[1]+")");
				//PLHotspot hotspot1 = new PLHotspot(++id, new PLImage(PLUtils.getBitmap(getApplicationContext(), R.raw.hotspot), false), py[0], py[1], 0.05f, 0.05f);
		        //panorama.addHotspot(hotspot1);
		        //Reset view
		        //reset();
		        //Load panorama
		       // startTransition(new PLTransitionBlend(2.0f), panorama); //or use this.setPanorama(panorama);
				
				/*panorama = (PLPanoramaBase) getPanorama();
				PLVector3 plVector3 = panorama.getCollisionData().hitPoint[0];
				float x = plVector3.x;
				float y = plVector3.y;
				float z = plVector3.z;
				Log.d("MainActivity", "(x,y,z)"+ x+ "," + y +","+ z);
				float[] pr = convertPositionToPitchAndYaw(y,z,x);
				panorama = new PLSphericalPanorama();
	            ((PLSphericalPanorama)panorama).setImage(new PLImage(PLUtils.getBitmap(getApplicationContext(), R.raw.quito1_s), false));
				panorama.getCamera().lookAt(0.0f, 0.0f);//璁剧疆鍨傜洿锛屾按骞�
		        //Add a hotspot
		        panorama.addHotspot(new PLHotspot(++id,new PLImage(PLUtils.getBitmap(MainActivity.this, R.raw.hotspot), false), pr[0], pr[1], 0.05f, 0.05f));
		        //Reset view
		        reset();
		        //Load panorama
		        startTransition(new PLTransitionBlend(2.0f), panorama); //or use this.setPanorama(panorama);*/
			...
			public void onDidBeginCameraAnimation(PLIView view, Object sender, PLICamera camera, PLCameraAnimationType type)
			{
				...
				Log.d(TAG," onDidBeginCameraAnimation patch = " + camera.getPitch());
				Log.d(TAG," onDidBeginCameraAnimation yaw = " + camera.getYaw());
			}
			...
			public void onDidEndCameraAnimation(PLIView view, Object sender, PLICamera camera, PLCameraAnimationType type)
			{
				...
				Log.d(TAG," onDidEndCameraAnimation patch = " + camera.getPitch());
			...
			@Override
			public void onDidRotateCameraDelta(PLIView view, Object sender, PLICamera camera, float pitchDelta, float yawDelta)
			{
				// TODO Auto-generated method stub
		        ...
		        //reset();
		        //Load panorama
		        //startTransition(new PLTransitionBlend(0.1f), panorama); //or use this.setPanorama(panorama);
			}
	...
	private void loadPanorama(int index)
    {
		try
		{
	    	...
	    	if(panorama != null)
	    	{
	    		//Set camera rotation
	    		panorama.getCamera().lookAt(0.0f, 0.0f);//璁剧疆鍨傜洿锛屾按骞�
	    		Log.d(this.getClass().getSimpleName(), "camera(x,y,z)" +panorama.getCamera().getX() + "," + panorama.getCamera().getY() + "," + panorama.getCamera().getZ());
	    		panorama.getCamera().getX();
	    		panorama.getCamera().getY();
	    		panorama.getCamera().getZ();
		        //Add a hotspot
	    		PLHotspot hotspot1 = new PLHotspot(++id, new PLImage(PLUtils.getBitmap(context, R.raw.hotspot), false), 0.0f, 0.0f, 0.05f, 0.05f);
		        panorama.addHotspot(hotspot1);
	    		//PLHotspot hotspot2 = new PLHotspot(++id, new PLImage(PLUtils.getBitmap(context, R.raw.hotspot), false), 0.0f, 0.0f + id*10, 0.05f, 0.05f);
		        //panorama.addHotspot(hotspot2);
		        //PLHotspot hotspot3 = new PLHotspot(++id, new PLImage(PLUtils.getBitmap(context, R.raw.hotspot), false), 0.0f, 0.0f + id*10, 0.05f, 0.05f);
		        //panorama.addHotspot(hotspot3);
		        //PLHotspot hotspot4 = new PLHotspot(++id, new PLImage(PLUtils.getBitmap(context, R.raw.hotspot), false), 0.0f, 0.0f + id*10, 0.05f, 0.05f);
