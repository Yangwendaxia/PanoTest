...
public class PLHotspot extends PLSceneElementBase implements PLIHotspot
{
	...
	{
		Log.d(this.getClass().getSimpleName()," orgPitch = " + pitch + " orgYaw = " + yaw);
		float r = this.getZ(), pr = (90.0f - pitch) * PLConstants.kToRadians, yr = -yaw * PLConstants.kToRadians;
		Log.d(this.getClass().getSimpleName(), "r = " + r);
	...
	public static float[] convertPositionToPitchAndYaw(float x, float y,float z)
	{
		...
		float pr = (float) Math.acos(y);
		float yr = (float) Math.atan(x/z);
		
		float pitch= 90-pr/PLConstants.kToRadians;
		float yaw =-yr/PLConstants.kToRadians;
		
		float[] py = new float[]{pitch,yaw};
		Log.d("PLHotspot"," pitch = " + py[0] + " yaw = " + py[1]);
		return py;
	}
	
	/**add by myself*/
	public static float[] convertPositionToPitchAndYaw2(float x, float y,float z)
	{
